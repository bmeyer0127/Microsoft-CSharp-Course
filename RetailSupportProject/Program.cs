// This is a retail application that will manage the money in a till
// This application will meet the following specifications:
...
static void MakeChange(int cost, int[] cashTill, int twenties, int tens = 0, int fives = 0, int ones = 0)
{
  ...
  // Count amount of each demonination taken
  int twentiesTaken = 0;
  int tensTaken = 0;
  int fivesTaken = 0;
  int onesTaken = 0;
  ...
  if (changeNeeded > 0)
  {
    // Amend till to previous state before throwing exception

    // Take back change given
    cashTill[3] += twentiesTaken;
    cashTill[2] += tensTaken;
    cashTill[1] += fivesTaken;
    cashTill[0] += onesTaken;

    // Give customer back money
    cashTill[3] -= twenties;
    cashTill[2] -= tens;
    cashTill[1] -= fives;
    cashTill[0] -= ones;
